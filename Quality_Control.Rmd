...
processCellRangerOutput <- function(patientName, emptyDropsIterations, save, returnSCE) {
  ...
  ## 1SCC = 100000, 1HS = 10000, 2HS = 50000, 2AK = 100000
  e.out <- emptyDrops(counts(sce), niters = emptyDropsIterations, test.ambient=TRUE)
  ...
  DataFrame(LibSize=sum(qc.lib2), NExprs=sum(qc.nexprs2), MitoProp=sum(discard.mito), Total=sum(discard))
  ...
  if (isTRUE(save)) {
    ...
    ifelse(!dir.exists(file.path(.path, "clean_data")), test=dir.create(file.path(.path, "clean_data")), FALSE)
    
    # filename of sce 
    filename <- file.path(.path, "clean_data", paste0(patientName,"_cleanData_sceRDS"))
    
    # if filename doesn't exist already, create it and sace sce
    if (isFALSE(overwrite) & file.exists(filename)) {
      warning("File already exists! Skip writting file ", filename)
    } else {
