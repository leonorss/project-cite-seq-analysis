#######################################################################################
# Quality check for later scRNAseq analysis of Levesque’s data
# Autors: Leonor Schubert, Jonathan Haab, Flavio Rump
# Autumn 2020
# Course STA426, UZH
#######################################################################################
processCellRangerOutput <- function(patientName, emptyDropsIterations, save, returnSCE, removeEmptyDroplets, cores=4) {
  ...
  if (isTRUE(removeEmptyDroplets)){
    ...
    e.out <- emptyDrops(counts(sce), niters = emptyDropsIterations, test.ambient=TRUE, BPPARAM=MulticoreParam(cores))
