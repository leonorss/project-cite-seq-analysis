...
A common type of skin cancer is called squamous cell carcinoma (SCC). This is the second most common form of skin cancer and afflicts roughly 1M Americans a year. The main causes of SCC are mutations arising from UV radiation. @SkinCancerFacts2020
...
# the known_markers are from the literature
...
We started by cleaning up the raw data and doing some clustering to identify the general cell types mentioned above. Then we focused on the composition of the two cell types fibroblasts and keratinocytes. Additionally we had a look at the cell dynamics of the keratynocites. What appears from all this analysis, is that there seems to be a difference between the healthy and cancer samples in both cell type and sub-cell type composition. This seems to be particuarly true for the lymphocytes and the keratinocytes. But it is important to keep in mind, that we only have the data of two patients and therefore not a very statistically reliable result. Also as mentioned before, e.g. clusters that have no strong expression for any of the known markers we looked at, could have an effect on the number of cells counted towards a certain cell type. This could potentially give us a false impression on how different a cell type is in the healthy and cancer sample. As mentioned before this would be something to look into some more, but the assumption is that it won't have that much of an effect.
