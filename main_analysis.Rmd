...
Hence, we looked for alternative approaches. At Mark's suggestion, we implemented SingleR @SingleR, which assigns each cell to a cell type individually. It does this by leveraging previously published data about expression profiles of pure cells and their corresponding cell type . In this case we are using the celldex package and tried out both the BluePrintEncode Data set as well as the HumanPrimaryCellAtlasData. This original data was generated by @Martens2013Oct and @Mabbott2013Dec respectively and then further processed as in @Aran2019Feb. 
...
```{r reference-clustering-human_cell_atlas,eval=FALSE}
...
```{r keratinocyte_dynamics , fig.keep=c(5,6)}
...
The results on the keratinocytes have been susceptible to small changes in parameters, so we don't trust these results very much.

<!-- #### Fibroblast subcomposition

```{r fibroblast, results='hide', fig.keep=c(5, 10, 15, 20), eval=FALSE, include=FALSE}
...
We started by cleaning up the raw data and applying clustering algorithm to identify the cell type populations mentioned above. With the pseudobulk method, the clustering of main cell type appeared to be quite satisfying whereas the cell subtypes were more challenging to separate. We observed differences in the cell (sub)type proportion in the different samples, in particular for lymphocytes and keratinocytes. The general trend showed an increase of lymphocytes and a decrease of keratinocytes in cancer tissue. The p-values for those analyses confirm that those results are unlikely to have been obtained by chance.

Howevever, when we used cell type assignment by a reference genome set, we got different results, where for example the proportion of lymphocytes is increased, but not nearly as much as with the pseudobulk method.

In the end, the pseudobulk clustering offers a generalization that has its value and researches need to decide how to integrate cell type assignment using more granular versions when making their conclusions.

## Discussion

The first point which should be discussed is related to the few relatively arbitrary cut-off values that were used. To be more specific, we decided to use the 20 first principal components (PCs) and to build our kNN graph linking a cell to its 15 closest neighbors. Both factors are influencing the results and can be discussed.
...
We recommend a more in depth investigation by using and comparing different methods in order to make sure that analysis and conclusions are reliable.

### Impact for therapeutic interventions
