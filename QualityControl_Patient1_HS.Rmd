---
title: "QualityControl_Patient1_HS"
...
## Should we really discard all these cells (667633), would only keep 69647?
...
lost.patient1_HS <- calculateAverage(counts(sce.patient1_HS)[, !discard.patient1_HS])
kept.patient1_HS <- calculateAverage(counts(sce.patient1_HS)[, discard.patient1_HS])

logged.patient1_HS <- edgeR::cpm(cbind(lost.patient1_HS, kept.patient1_HS), log=TRUE, prior.count=2)
logFC.patient1_HS <- logged.patient1_HS[, 1] - logged.patient1_HS[, 2]
abundance.patient1_HS <- rowMeans(logged.patient1_HS)

plot(abundance.patient1_HS, logFC.patient1_HS, xlab="Average count", ylab="Log-FC (lost/kept)", pch=16)
```
