...
sce.patient1_HS <- remove_rare_genes(sce.patient1_HS, 4)
...
sc_cluster <- function(sceo, k=30) {
  ...
  plots <- plot_grid( plotTSNE(sceo, colour_by="cluster", text_by="cluster"), plotUMAP(sceo, colour_by="cluster", text_by="cluster") )
  print(plots)
  return(list(sceo, go))
...
annotate_cells <- function(sceo, go) {
...
results <- annotate_cells(sce.patient1_HS, g.patient1_HS)
...
pseudobulk <- function(sceo, kmo) {
  ...
  print(h1)
  p <- plotUMAP(sceo, colour_by="cluster2", text_by="cluster2", point_size=1)
  print(p)
...
sce.patient1_HS.kera <- remove_rare_genes(sce.patient1_HS.kera, 4)
results <- variance_stabilization(sce.patient1_HS.kera, 2000)

sce.patient1_HS.kera <- results[[1]]
hvg.patient1_HS.kera <- results[[2]]
...
sce.patient1_HS.kera <- sc_PCA(sce.patient1_HS.kera, hvg.patient1_HS.kera, known_markers.kera)

#--- clustering

results <- sc_cluster(sce.patient1_HS.kera)
sce.patient1_HS.kera <- results[[1]]
...
results <- annotate_cells(sce.patient1_HS.kera, g.patient1_HS.kera)

sce.patient1_HS.kera <- results[[1]]
km.patient1_HS.kera <- results[[2]]

#---

results <- pseudobulk(sce.patient1_HS.kera, km.patient1_HS.kera)

sce.patient1_HS.kera <- results[[1]]
pb.patient1_HS.kera <- results[[2]]
mat.patient1_HS.kera <- results[[3]]
cl2.patient1_HS.kera <- results[[4]]
